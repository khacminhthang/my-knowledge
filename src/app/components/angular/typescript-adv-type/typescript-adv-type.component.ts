...
export class TypescriptAdvTypeComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.text1 = `function listen(port: unknown) {
  if (typeof port === 'string') {
    port = parseInt(port, 10);
  }
  server.listen(port);
}`;
    this.text2 = `typeof 'string'; // string
typeof 123; // number
typeof true; // boolean
typeof {}; // object
typeof []; // object
...
typeof typeof 123; // string`;
    this.text3 = `function listen(port: string | number) {
  // do listen
}
