...
export class RxjsCreationOperatorsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.text1 = `const observable = new Observable(function subscribe(observer) {
  const id = setTimeout(() => {
  ...
  return function unsubscribe() {
    ...
    this.text2 = `const observer = {
  next: (val) => console.log(val),
  error: (err) => console.log(err),
  complete: () => console.log('complete'),
};`;
    this.text3 = `// output: 'hello'
// complete: 'complete'
of('hello').subscribe(observer);`;
    this.text4 = `// output: [1, 2, 3]
// complete: 'complete'
of([1, 2, 3]).subscribe(observer);`;
    this.text5 = `// output: 1, 2, 3, 'hello', 'world', {foo: 'bar'}, [4, 5, 6]
// complete: 'complete'
of(1, 2, 3, 'hello', 'world', { foo: 'bar' }, [4, 5, 6]).subscribe(observer);`;
    this.text6 = `// output: 1, 2, 3
// complete: 'complete'
from([1, 2, 3]).subscribe(observer);`;
    this.text7 = `// output: 'h', 'e', 'l', 'l', 'o', ' ', 'w', 'o', 'r', 'l', 'd'
// complete: 'complete'
from('hello world').subscribe(observer);`;
    this.text8 = `const map = new Map();
map.set(1, 'hello');
map.set(2, 'bye');
...
from(set).subscribe(observer);`;
    this.text9 = `// output: 'hello world'
// complete: 'complete'
from(Promise.resolve('hello world')).subscribe(observer);`;
    this.text10 = `const btn = document.querySelector('#btn');
...
fromEvent(input, 'keydown').subscribe(observer);`;
    this.text11 = `// fromEvent() từ ví dụ trên
// output: MouseEvent {...}
fromEvent(btn, 'click').subscribe(observer);
...
fromEventPattern(
  ...
  (ev: MouseEvent) => ev.offsetX + ' ' + ev.offsetY
    ...
    this.text13 = `// _getHub() là hàm trả về Hub.
