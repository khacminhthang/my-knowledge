...
export class IntroRxjsObservableComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.text1 = `const btnjsThrottle = document.querySelector("#jsThrottle");
...
fromEvent(btnrxjsThrottle, 'click')
  ...
  .subscribe(count => console.log(RxJS: Clicked $count times));`;
    this.text2 = `const observable = new Observable(function subscribe(observer) {
  const id = setTimeout(() => {
    ...
    this.text3 = `const observable = new Observable(function subscribe(observer) {
  const id = setTimeout(() => {
    observer.next('Hello Rxjs');
    observer.complete();
  }, 1000);
  return function unsubscribe() {
    ...
    this.text4 = `const subscription = observable.subscribe({
  next: (value) => {
    console.log(value);
  },
  error: (error) => {
  ...
  complete: () => {
    ...
    this.text5 = `const subscription = observable.subscribe({
  next: (value) => {
    console.log(value);
  },
  error: (error) => {
...
setTimeout(() => {
  subscription.unsubscribe();
    ...
    this.text6 = `const observer = {
  next: x => console.log('Observer got a next value: ' + x),
  error: err => console.error('Observer got an error: ' + err),
  complete: () => console.log('Observer got a complete notification'),
};`;
    this.text7 = `observable.subscribe(observer);`;
    this.text8 = `observable.subscribe(
...
observable.subscribe(observer);`;
    this.text9 = `observable.subscribe(
  x => console.log('Observer got a next value: ' + x),
  null,
  () => console.log('Observer got a complete notification')
);`;
    this.text10 = `const observable = interval(1000);
const subscription = observable.subscribe(x => console.log(x));

setTimeout(() => {
subscription.unsubscribe();
}, 5000);`;
    this.text11 = `const foo = interval(500);
