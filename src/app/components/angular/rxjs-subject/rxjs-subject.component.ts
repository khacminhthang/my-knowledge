...
export class RxjsSubjectComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.text1 = `const observable = interval(500).pipe(
  take(5)
);
...
Observer A complete
*/`;
    this.text2 = `const observable = interval(500).pipe(
  take(5)
);
...
Observer B complete
*/`;
    this.text3 = `const hybridObserver = {
  observers: [],
...
Observer B complete
/*`;
    this.text4 = `const hybridObserver = {
  observers: [],
  subscribe(observer) {
    this.observers.push(observer);
  },
...
setTimeout(() => {
  hybridObserver.subscribe(observerB);
    ...
    this.text5 = `const subject = new Subject();
...
export class AppComponent implements OnInit  {
  ...
  onInput(event: Event) {
    ...
    this.text7 = `const subject = new Subject();
...
observerB: 3
*/`;
    this.text8 = `const subject = new BehaviorSubject(0); // 0 is the initial value
...
observerB: 3
*/`;
    this.text9 = `const subject = new ReplaySubject(3); // buffer 3 values for new subscribers
...
observerB: 5
*/`;
    this.text10 = `const subject = new ReplaySubject(100, 500 /* windowTime */);
...
...
*/`;
    this.text11 = `const subject = new AsyncSubject();
...
observerB: 5
*/`;
    this.text12 = `const subject = new BehaviorSubject(0); // 0 is the initial value
...
observerC: done
*/`;
    this.text13 = `const subject = new ReplaySubject(3);
...
observerB: done
*/`;
    this.text14 = `const subject = new AsyncSubject();
...
observerB: done
*/`;
    this.text15 = `const observable = interval(500).pipe(
  take(5)
);
...
setTimeout(() => {
  subject.subscribe(observerB);
    ...
    this.text16 = `const subject = new Subject();
...
Observer B complete
*/`;
    this.text17 = `const subject = new Subject();
...
log.info: 10
...`;
    this.text18 = `const sub = connectableObservable.subscribe(observerA);
...
setTimeout(() => {
  ...
  connectSub.unsubscribe();
    ...
    this.text19 = `const subject = new Subject();
...
setTimeout(() => {
  subB.unsubscribe(); // ref from 1 => 0
    ...
    this.text20 = `const connectableObservable = interval(500).pipe(
  take(10),
  tap(x => console.log('log.info: ' + x)),
...
setTimeout(() => {
  const subA2 = sharedObservable.subscribe(observerA);
    ...
    this.text21 = `const connectableObservable = interval(500).pipe(
  take(10),
  tap(x => console.log('log.info: ' + x)),
  multicast(() => new Subject())
) as ConnectableObservable<number>;`;
    this.text22 = `const sharedObservable = connectableObservable.refCount();
...
setTimeout(() => {
  const subA2 = sharedObservable.subscribe(observerA);
    ...
    this.text23 = `const connectableObservable = interval(500).pipe(
  tap(x => console.log('log.info: ' + x)),
  publish(),
) as ConnectableObservable<number>;
...
setTimeout(() => {
  subB.unsubscribe(); // ref from 1 => 0
    ...
    this.text24 = `const sharedObservable = interval(500).pipe(
  tap(x => console.log('log.info: ' + x)),
  share(),
);
...
setTimeout(() => {
  subB.unsubscribe(); // ref from 1 => 0
    ...
    this.text25 = `import { Injectable } from '@angular/core';
