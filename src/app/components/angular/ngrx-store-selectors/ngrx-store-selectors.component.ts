...
export class NgrxStoreSelectorsComponent implements OnInit {
  ...
  text1 = `import { createSelector } from '@ngrx/store';
...
export const selectFeatureCount = createSelector(
  ...
  text2 = `import { createSelector } from '@ngrx/store';
...
export const selectVisibleBooks = createSelector(
  ...
  text3 = `export const selectCount = createSelector(
  selectCounterValue,
  ...
  text5 = `export const selectCount = () =>
  createSelector(
  ...
  text7 = `import { createSelector, createFeatureSelector } from '@ngrx/store';
...
export const selectFeatureCount = createSelector(
  ...
  text8 = `import { createSelector } from '@ngrx/store';
...
selectTotal(state); // computes the sum of 3 & 5, returning 8. selectTotal now has a memoized value of 8`
  text9 = `selectTotal(state); // returns the memoized value of 8
selectTotal.release(); // memoized value of selectTotal is now null`
  text10 = `export interface State {
  evenNums: number[];
  oddNums: number[];
}
 ...
 *   selectTotal        null
 */`
  text11 = `export class AppComponent {
  counter$ = this.store.select(fromCounter.selectCounter);

  constructor(private readonly store: Store) {}
}`
  text12 = `import { createSelector, createFeatureSelector } from '@ngrx/store';
...
export const selectFeatureCount = createSelector(
  ...
  text13 = `import { Component, inject } from '@angular/core';
...
export class UsersComponent {
  ...
  text14 = `import { map, filter } from 'rxjs/operators';
...
store
  ...
  .subscribe(/* .. */);`
  text15 = `import { select } from '@ngrx/store';
...
.subscribe(/* .. */);`
  text16 = `export const selectProjectedValues = createSelector(
  selectFoo,
  selectBar,
  (foo, bar) => {
    if (foo && bar) {
  ...
  text17 = `// The number of state transitions is given by the user (subscriber)
...
   scan((acc, curr) => {
  ...
  text18 = `// Subscribe to the store using the custom pipeable operator
