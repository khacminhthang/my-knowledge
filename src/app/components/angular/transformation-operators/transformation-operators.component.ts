...
export class TransformationOperatorsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.text1 = `observableInstance.pipe(
  operator1(),
  operator2(),
)`;
    this.text2 = `const returnObservable = observableInstance.pipe(
  operator1(),
  operator2(),
)`;
    this.text3 = `const users = [
...
const usersVm = users.map(user => {
  ...
  },
  {
    ...
    this.text5 = `import { Observable } from 'rxjs';
...
source.subscribe(observer);`;
    this.text6 = `import { map } from 'rxjs/operators';
...
source.pipe(
  map(user => {
    ...
    this.text7 = `source.pipe(
  map(user => user.id)
).subscribe(observer);`;
    this.text8 = `import { pluck } from 'rxjs/operators';

source.pipe(
  pluck('id')
).subscribe(observer);`;
    this.text9 = `const element = document.querySelector('#hover');
...
hover$.subscribe(observer);`;
    this.text10 = `const button = document.querySelector('#add');
...
click$.pipe(
  scan((acc, curr) => acc + 1, 0)
    ...
    this.text11 = `const users$ = new Observable<User>((observer) => {
  const users = [
...
users$.pipe(
  scan((acc, curr) => acc + curr.postCount, 0)
    ...
    this.text12 = `users$.pipe(
  reduce((acc, curr) => acc + curr.postCount, 0)
).subscribe(observer);`;
    this.text13 = `users$.pipe(
  reduce((acc, curr) => [...acc, curr], [])
).subscribe(observer);`;
    this.text14 = `users$.pipe(
  toArray()
).subscribe(observer)`;
    this.text15 = `const interval$ = interval(1000);
...
"Buffered Values: "
[2, 3, 4, 5, 6]`;
    this.text16 = `const source = interval(500);
...
// output: 0.4146530439875191
now$.subscribe(observer);`;
    this.text18 = `const now$ = defer(() => of(Math.random()));
