...
export class LoggingComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.text1 = `public class SchoolContext : DbContext
{
    ...
    this.text2 = `public static readonly ILoggerFactory loggerFactory = new LoggerFactory(
  new[] { new ConsoleLoggerProvider ((_, __) => true, true) }
);`;
    this.text3 = `new ConsoleLoggerProvider((_, __) => true, true)`;
    this.text4 = `using (var context = new SchoolContext())
{
    ...
    this.text5 = `dbug: Microsoft.EntityFrameworkCore.Infrastructure[100401]
...
Disposing transaction.`;
    this.text6 = `public static readonly ILoggerFactory consoleLoggerFactory
= new LoggerFactory(new[] {
      new ConsoleLoggerProvider((category, level) =>
        category == DbLoggerCategory.Database.Command.Name &&
        level == LogLevel.Information, true)
    });`;
    this.text7 = `public static readonly ILoggerFactory consoleLoggerFactory
= new LoggerFactory().AddConsole();`;
    this.text8 = `SELECT TOP(1) [s].[StudentId], [s].[DoB], [s].[FirstName], [s].[GradeId], [s].[LastName],
...
Go`;
    this.text9 = `var context = new SchoolContext();

var student = context.Students.Where(s => s.FirstName == "Bill")
                        .Include(s => s.Grade)
                            .ThenInclude(g => g.Teachers)
                        .FirstOrDefault();`;
    this.text10 = `SELECT TOP(1) [s].[StudentId], [s].[DoB], [s].[FirstName], [s].[GradeId], [s].[LastName],
[s].[MiddleName], [s.Grade].[GradeId], [s.Grade].[GradeName], [s.Grade].[Section]
FROM [Students] AS [s]
LEFT JOIN [Grades] AS [s.Grade] ON [s].[GradeId] = [s.Grade].[GradeId]
WHERE [s].[FirstName] = N'Bill'
...
ORDER BY [t0].[GradeId]
go`;
    this.text11 = `var context = new SchoolContext();
...
<pre>{{ signInForm.value | json }}</pre>
    ...
    this.text13 = `onSubmit(form: NgForm) {
  // Do something awesome
  console.log(form);
  throw new Error('something went wrong');
}`;
    this.text14 = `export class SignInComponent implements OnInit {
  userInfo = {
    userName: 'tiepphan',
    password: '',
    rememberMe: true,
...
// third (third complete vì take(2))`;
    this.text19 = `const formControlValueChanges = Object.keys(this.formGroup.value).map((key) =>
...
// output: fast - 1s -> fast - 1s -> fast - 1s -> fast...`;
  ...
  .subscribe(() => this.closeBanner());`;
    this.text22 = `fromEvent(document, 'click')
...
// - click lúc 5.5s -> [MouseEvent, 4]; // sau 5s thì giá trị gần nhất của interval(1000) là 4 (0 - 1 - 2 - 3 -4)`;
    this.text23 = `this.apiService.getSomething().pipe(withLatestFrom(this.currentLoggedInUser$));
// các bạn gọi một API và các bạn muốn dùng kết quả của API này + với thông tin của người dùng đang đăng nhập để thực hiện nghiệp vụ ké tiếp`;
    this.text24 = `of('world').pipe(starWith('Hello')).subscribe(observer);
// output:
// 'Hello'
// 'word'
// 'complete'`;
    this.text25 = `this.books$ = this.apiService.getBooks().pipe(startWith([]));
...
<!-- vì books$ đã có startWith([]) nên giá trị của books sẽ là truthy ngay từ đầu, nên *ngIf này sẽ truthy và render bên trong ng-container ngay -->
    ...
    this.text26 = `of('hi', 'how are you?', 'sorry, I have to go now')
