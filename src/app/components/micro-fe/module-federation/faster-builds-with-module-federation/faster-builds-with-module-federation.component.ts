...
export class FasterBuildsWithModuleFederationComponent implements OnInit {
  ...
  text1 = `# Replace acme with desired scope
npx create-nx-workspace acme --preset=apps
cd acme`
  text2 = `# If you use React
nx add @nx/react
...
nx add @nx/angular`
  ...
  text6 = `// apps/host/project.json
{
  //...
  "implicitDependencies": ["about", "shop", "cart"]
}`
  text7 = `// For Angular, you'll see @nx/angular/module-federation
...
export default withModuleFederation({
  ...
  text8 =  `import { withModuleFederation } from '@nx/angular/module-federation';
...
export default async function (wco) {
  ...
  text9 = `import { ModuleFederationConfig } from '@nx/webpack';
...
export default config;`
  text10 = `// module-federation.config.ts
...
export const config: ModuleFederationConfig = {
  ...
  text11 = `nx serve host

# (kill server)

nx serve host`
  text12 = `> nx run about:build:development  [existing outputs match the cache, left as is]
...
"implicitDependencies": ["about", "shop", "cart"]
  ...
  text14 = `import { withModuleFederation } from '@nx/react/module-federation';
import moduleFederationConfig from './module-federation.config';
...
├── host
└── shop`
  text16 = `nx g @nx/workspace:run-commands \
deploy \
--project=host \
